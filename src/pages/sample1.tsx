                            ...
                            <Box display="flex" alignItems="stretch" gap={2}>
                                <Box sx={{ flexGrow: 1 }}>
                                    <Stack spacing={2}>
                                        ...
                                        {renderAlertForStatus("FFmpeg", ffmpegStatus)}
                                    </Stack>
                                </Box>
                                <Box>
...
const LogView: React.FC<LogViewProps> = ({ log, isProcessing, onReset }) => (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', p: 3 }}>
        <Typography variant="h5" gutterBottom>Execution Log</Typography>
        <Paper variant="outlined" sx={{ flexGrow: 1, overflowY: 'auto', p: 2, color: 'common.white', backgroundColor: 'common.black', fontFamily: 'monospace' }}>
            {log.map((line, index) => <div key={index}>{line}</div>)}
            {isProcessing && <CircularProgress size={24} sx={{ mt: 1 }} />}
        </Paper>
        <Button variant="contained" color="success" sx={{ mt: 2 }} onClick={onReset} disabled={isProcessing} startIcon={<RestartAltIcon />}>Return to Start</Button>
    </Box>
);
